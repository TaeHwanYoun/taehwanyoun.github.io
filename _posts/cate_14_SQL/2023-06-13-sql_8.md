...
|  | COUNT(열 이름) : NULL값을 제외한 행의 수 |
|  | COUNT(DISTINCT(열 이름)) : NULL값을 제외한 행의 수 |
| **SUM** | SUM(열 이름) : 전체 합계 |
| **AVG** | AVG(열 이름) : 평균 |
| **MAX** | MAX(열 이름) : 최댓값 |
| **MIN** | MAX(열 이름) : 최솟값 |
| **STDENV or STD** | STDENV(열 이름) : (Orcle or Mysql)표준편차 |
| **VARIANCE** | VARIANCE(열 이름) : 분산 |
